...
public class SwitchLabelAnnotation
{
    ...
    public static ProgramNode Annotate(ProgramNode node)
        =>  node with { Function = new SwitchLabelAnnotation().Function(node.Function, null, null) };
        
    private FunctionNode Function(FunctionNode node, string? label, List<SwitchLabel>? cases)
        => node with { Body = Block(node.Body, label, null) };

    [return: NotNullIfNotNull(nameof(statement))]
    private IStatementNode? Statement(IStatementNode? statement, string? label, List<SwitchLabel>? cases)
        => statement switch
        {
            IfNode node => If(node, label, cases),
            LabelNode node => Label(node, label, cases),
            ...
            CompoundNode node => Compound(node, label, cases),
            CaseNode node => Case(node, label, cases),
            DefaultNode node => Default(node, label, cases),
    ...
    private static BreakNode Break(BreakNode node, string? label)
    {
        ...
        // label from an outer loop, so we'll have to replate it.
        if (label is null) return node;
        return node with { Label = label };
    }

    private DefaultNode Default(DefaultNode node, in string? label, List<SwitchLabel>? cases)
    {
        ...
        return node with { Label = defaultLabel, Statement = Statement(node.Statement, label, cases)};       
    }

    private CaseNode Case(CaseNode node, in string? label, List<SwitchLabel>? cases)
    {
        ...
        {
            Label = caseLabel, 
            Statement = Statement(node.Statement, label, cases)
        };
    ...
    private SwitchNode Switch(SwitchNode node)
    {
        ...
        var body = Statement(node.Body, label, cases);
    ...
    private WhileNode While(WhileNode node, string? label, List<SwitchLabel>? cases)
        => node with { Body = Statement(node.Body, label, cases) };
    ...
    private LabelNode Label(LabelNode node, string? label, List<SwitchLabel>? cases)
        => node with { Statement = Statement(node.Statement, label, cases) };

    private IfNode If(IfNode node, string? label, List<SwitchLabel>? cases)
        => node with { Then = Statement(node.Then, label, cases), Else = Statement(node.Else, label, cases) };

    private CompoundNode Compound(CompoundNode node, string? label, List<SwitchLabel>? cases)
        => node with { Block = Block(node.Block, label, cases) };

    private BlockNode Block(BlockNode node, string? label, List<SwitchLabel>? cases)
    {
        var items = node.Items
            .Select(i => i is IStatementNode s ? Statement(s, label, cases) : i)
            .ToList();
