...
public class FunctionData : DataBase
{
    public FunctionData()
    {
        ...
        (
            ...
            GetExpected([
                ...
                new FunctionDeclarationNode("main", "int", [], new BlockNode([
                    ...
                    new ExpressionNode(new SubtractionAssignmentNode(Var("x"), new FunctionCallNode("foo", []))),
                    Ret(Var("x"))
                ]))
            ])
        );
        ...
        (
            ...
            GetExpected([
                new FunctionDeclarationNode("main", "int", [], new BlockNode([
                    ...
                    new VariableDeclarationNode("bar", Const(4)),
                    new IfNode
                    (
                        new BinaryNode
                        (
                            GreaterThanNode.Operator, 
                            new BinaryNode(AdditionNode.Operator, Var("foo"), Var("bar")), 
                            Const(0)
                        ),
                        Compound
                             (
                                 new FunctionDeclarationNode("foo", "int", [], null),
                                 new ExpressionNode(new AssignmentNode(Var("bar"), new FunctionCallNode("foo", [])))
                             )
                    ),
                    Ret(new BinaryNode(AdditionNode.Operator, Var("foo"), Var("bar")))
                ])),
        ...
        (
            ...
            """,
            GetExpected([
                new FunctionDeclarationNode("fib", "int", ["n"], new BlockNode([
                    new IfNode
                    (
                        new BinaryNode
                        (
                            LogicalOrNode.Operator,
                            new BinaryNode(EqualNode.Operator, Var("n"), Const(0)),
                            new BinaryNode(EqualNode.Operator, Var("n"), Const(1))
                        ),
                        Compound(Ret(Var("n"))),
                        Compound
                            (
                                ...
                                (
                                    ...
                                    (
                                        ...
                                        new FunctionCallNode("fib", [new BinaryNode(SubtractionNode.Operator, Var("n"), Const(1))]),
                                        new FunctionCallNode("fib", [new BinaryNode(SubtractionNode.Operator, Var("n"), Const(2))])
                                    )
                                )
                            )
                    )
        ...
        (
            ...
            GetExpected
            (
                new FunctionDeclarationNode("add", "int", ["x", "y"], new BlockNode([
                new ReturnNode(new BinaryNode(AdditionNode.Operator, Var("x"), Var("y")))]))
            )
        );
        Add
        (
            ...
            """,
            GetExpected([
                new FunctionDeclarationNode("add", "int", ["x", "y"], null),
                new FunctionDeclarationNode("main", "int", [], new BlockNode([
                    new ReturnNode
                    (
